...
class FaceIDAnimationController {
  ...
  FaceIDAnimationController({
    ...
    required this.customPaintSize,
        ...
        moveD1 = Tween<double>(begin: 0, end: 15).animate(CurvedAnimation(
          parent: controller,
          curve: const Interval(0.3, 0.4, curve: Curves.fastOutSlowIn),
        )),
        moveR2 = Tween<double>(begin: 0, end: 15).animate(CurvedAnimation(
          parent: controller,
          curve: const Interval(0.4, 0.6, curve: Curves.fastOutSlowIn),
        )),
        moveU2 = Tween<double>(begin: 0, end: -5).animate(CurvedAnimation(
          parent: controller,
          curve: const Interval(0.4, 0.6, curve: Curves.fastOutSlowIn),
        )),
        closeRRect = Tween<double>(begin: 0, end: 0.5).animate(CurvedAnimation(
          parent: controller,
          curve: const Interval(0.4, 0.6, curve: Curves.fastOutSlowIn),
        )),
        borderRadiusRRect =
            Tween<double>(begin: 50, end: 100).animate(CurvedAnimation(
          parent: controller,
          curve: const Interval(0.45, 0.6, curve: Curves.fastOutSlowIn),
        )),
        faceOpacity = Tween<double>(begin: 1, end: 0).animate(CurvedAnimation(
          parent: controller,
          curve: const Interval(0.825, 0.875, curve: Curves.fastOutSlowIn),
        )),
        check1X = Tween<double>(
                begin: customPaintSize * 0.275, end: customPaintSize * 0.5)
            .animate(CurvedAnimation(
          parent: controller,
          curve: const Interval(0.9, 0.95, curve: Curves.fastOutSlowIn),
        )),
        check1Y = Tween<double>(
                begin: customPaintSize * 0.5, end: customPaintSize * 0.7)
            .animate(CurvedAnimation(
          parent: controller,
          curve: const Interval(0.9, 0.95, curve: Curves.fastOutSlowIn),
        )),
        check2X = Tween<double>(
                begin: customPaintSize * 0.5, end: customPaintSize * 0.75)
            .animate(CurvedAnimation(
          parent: controller,
          curve: const Interval(0.95, 1, curve: Curves.bounceOut),
        )),
        check2Y = Tween<double>(
                begin: customPaintSize * 0.7, end: customPaintSize * 0.3)
            .animate(CurvedAnimation(
          parent: controller,
          curve: const Interval(0.95, 1, curve: Curves.bounceOut),
        ));
